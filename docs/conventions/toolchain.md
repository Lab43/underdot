...
Keep `typescript` inside `typescript-eslint`'s peer range. Rationale: the linter's type-aware rules run on the compiler's programmatic API, which TypeScript 7 does not ship, and npm only warns when a newer compiler is installed over the range. Rejected: TypeScript 7 for the build with 6 installed under an alias for lint, two compilers checking one repo for a language, error set, and emitted JavaScript that are the same in both.
