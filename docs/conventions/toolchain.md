...
Group the core package's modules into directories named after the spec each enforces: `src/configuration/` for `docs/specs/configuration.md`, `src/source-tree/` for `docs/specs/source-tree.md`, `src/build/` for `docs/specs/build.md`, and a directory for each later spec the same way. A module's spec marker names its directory's spec, and its test sits beside it. The package's entries stay at the root: `src/index.ts`, which a script imports, and `src/underdot.ts`, the command the `bin` field installs. `src/index.ts` holds the exported functions in the signatures the specs name, each a thin call into the module that does the work, so a module's own signature can take what the pipeline needs rather than what a script passes. An entry that exports nothing is named after what it installs, since it has no function to take a name from. Rationale: the spec a module enforces is the grouping its marker already carries, so a reader with a spec open knows where its code is, and no plan re-argues where a module goes. Rejected: grouping by pipeline stage, which has no anchor outside the code and moves as the pipeline does.
...
Keep `typescript` inside `typescript-eslint`'s peer range. Rationale: the linter's type-aware rules run on the compiler's programmatic API, which TypeScript 7 does not ship, and npm only warns when a newer compiler is installed over the range, after which lint refuses to run. Rejected: TypeScript 7 for the build with 6 installed under an alias for lint, two compilers checking one repo for a language, error set, and emitted JavaScript that are the same in both.
