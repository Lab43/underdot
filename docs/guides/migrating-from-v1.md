...
- **`slug` is replaced by `_url`.** The page's identity is its URL: `/` for the home page, `/about/` for `about/index.html`. Replace `'/' + slug` with `_url`, and `slug === ''` with `_url === '/'`. A helper that compared `slug` compares `_url` (see: docs/specs/source-tree.md, URLs).
...
- **A script that runs more than one build keeps a script.** A project that built a second output, from a staged copy of the source or from a second source, calls the exported build function from its own script for each, instead of the command (see: docs/specs/configuration.md, Programmatic use).
...
- **`dirname` is no longer a variable.** Helpers receive the current file's directory from the render context instead (see: docs/specs/templates.md, Built-in variables).
- **`template:` takes the name without its underscore.** `template: _fancy` becomes `template: fancy` (see: docs/specs/templates.md, Template resolution).
