...
module.exports = class Renderer {
  ...
  outputTree (destination) { return ({ tree, globals }) => {
    ...
    const resolveAndRender = (slug, template, content, ...metadatas) => {
      ...
      return this.render(ext, body, templateMetadata, globals, ...metadatas, {content, slug}).then((content) => {
        if (parentTemplate) {
    ...
    const promises = [];
    const processLevel = (slug, {children, metadata, template, filePath, content}) => {
      if (content) promises.push(async () => {
        const html = await resolveAndRender(slug, template, content, metadata, {filePath});
