...
module.exports = class Underdot {
  ...
  constructor ({
    ...
    plugins.reverse().forEach((plugin) => plugin({
      ...
      registerRenderer: this.renderer.registerRenderer,
      registerTemplateHelper: this.renderer.registerHelper,
      registerPagePlugin: this.pagePlugins.register,
      logger,
    }));

    // now that all the plugins have registered their template helpers we can initialize the renderers
  ...
  async build () {
    ...
    this.queue.addAll(pages.map((page) => async () => {
      ...
      const content = await this.renderer.render(ext, body, metadata, {slug});
      tree.addPage({
        slug,
        template,
        metadata,
